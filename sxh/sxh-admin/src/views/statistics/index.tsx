...
interface IProps {
	...
	setThemeConfig: (themeConfig: ThemeConfigProp) => void;
}
...
const Statistics: FC<IProps> = props => {
	...
	// @ts-ignore
	const { pvCount, userCount, starPayCount, articleCount, tutorialCount, collectCount, likeCount, readCount, commentCount } =
		allInfo;
	...
	useEffect(() => {
		const getPieRef = () => {
			...
			let myPieChart = echarts.init(pieChartRef.current as HTMLElement, isDark ? "dark" : "light");
			let option = {
				title: {
					text: "数据统计",
					left: "center"
	...
	}, [collectCount, likeCount, readCount, commentCount, isDark, resizeChart]);

	// 折线图数据加载
	useEffect(() => {
		const getPvUvRef = () => {
			console.log("当前的主题是", isDark ? "dark" : "light");
			if (chartRef.current && echarts.getInstanceByDom(chartRef.current)) {
				echarts.dispose(chartRef.current);
			}
			let myChart = echarts.init(chartRef.current as HTMLElement, isDark ? "dark" : "light");
	...
	}, [pvUvDate, pvDateCount, uvDateCount, isDark, resizeChart]);

	return (
		<div className={`statistics ${isDark ? "statistics--dark" : ""}`}>
			<ContentWrap className="content">
				<div className="statistics-header">
					<div className="statistics-title">数据看板</div>
					<div className="statistics-subtitle">核心指标与访问趋势</div>
				</div>
				<div className="statistics-all__wrap top-content">
					...
					<div className="item-center">
						<div className="gitee-traffic traffic-box">
							...
							<span className="traffic-name sle">VIP用户</span>
						</div>
					...
					<div className="item-right">
						<div className="panel-title">互动构成</div>
						<div className="statistics-pie" ref={pieChartRef}></div>
					</div>
				</div>
				<div className="statistics-panel">
					<div className="statistics-panel__head">
						<div className="statistics-panel__title">访问趋势</div>
						<div className="statistics-setting">
							<Switch
								className="theme-switch"
								checked={isDark}
								onChange={checked => setThemeConfig({ ...themeConfig, isDark: checked })}
								checkedChildren="深色"
								unCheckedChildren="浅色"
							/>

							<Select className="day-select" value={pvUvDay} onChange={value => setPvUvDay(value)} options={dayLimitList} />
						</div>
					</div>
