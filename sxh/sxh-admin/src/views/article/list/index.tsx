			...
			render(value, item) {
				...
				return (
					<Tooltip title={tooltipContent}>
						<div>
							<a href={fullUrl} className="cell-text" target="_blank" rel="noreferrer">
								{value}
							</a>
							{shortTitle && <div style={{ fontSize: "12px", color: "#1890ff", marginTop: "4px" }}>教程: {shortTitle}</div>}
							{urlSlug && <div style={{ fontSize: "12px", color: "#999", marginTop: "4px" }}>{urlSlug}</div>}
						</div>
					</Tooltip>
				);
			}
