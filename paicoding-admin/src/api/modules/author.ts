...
export const getZsxqWhiteListApi = (data: { pageNumber: number; pageSize: number }) => {
	return http.post(`${PORT1}/author/whitelist`, data);
};

// 更新知识星球用户白名单
export const updateZsxqWhiteApi = (params: object | undefined) => {
	return http.post<Login.ResAuthButtons>(`${PORT1}/author/whitelist/save`, params);
};

// 审核知识星球用户白名单
export const operateZsxqWhiteApi = (params: object | undefined) => {
	return http.get<Login.ResAuthButtons>(`${PORT1}/author/whitelist/operate`, params);
};

// 审核知识星球用户白名单，批量
export const operateBatchZsxqWhiteApi = (params: object | undefined) => {
	return http.post<Login.ResAuthButtons>(`${PORT1}/author/whitelist/batchOperate`, params);
};
