...
import { getAuthorWhiteListApi, removeAuthorWhiteApi, updateAuthorWhiteApi } from "@/api/modules/author";
...
const AuthorWhiteList: FC<IProps> = props => {
	...
	const handleDel = (userId: number) => {
		Modal.warning({
			...
			closable: true,
			onOk: async () => {
				const { status } = await removeAuthorWhiteApi(userId);
	...
	const reviseDrawerContent = (
		<Form name="basic" form={formRef} labelCol={{ span: 4 }} wrapperCol={{ span: 16 }} autoComplete="off">
			<Form.Item label="作者" name="author" rules={[{ required: true, message: "请选择作者!" }]}>
				<AuthorSelect authorName={authorName} handleChange={handleChange} handleFormRefChange={() => {}} />
			</Form.Item>
		</Form>
	);
