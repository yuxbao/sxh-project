<template>
  ...
  <section class="article-info-wrap com-2-panel col-2-article J-articlePanel">
    ...
    <div>
      <div class="detail-content-title-other-wrap relative flex justify-between"
        v-if="articleVo.article && Object.keys(articleVo.article).length !== 0">
        <div id="article-basic-info" class="flex justify-between" style="color: #999999">
          <div class="flex">
            ...
            <p class="center-content mr-2">
              <RouterLink v-if="articleVo.article.author" :to="'/user/' + articleVo.article.author"
                class="detail-content-title-other-name el-link el-link--primary">
          ...
          <span class="p-1" v-if="articleVo.article.status !== 1" style="padding-left: 1em; color: red">{{
            articleVo.article.status == 0 ? '(草稿)' : '(审核中)' }}</span>
          <div class="flex" v-if="global.isLogin && articleVo.article.author == global.user.id">
            <p @click="() => { router.push('/article/edit/' + articleVo.article.articleId) }"
              class="p-1 edit-delete-btn flex">
        ...
        <!--    文章的tag    -->
        <div>
          <el-tag class="m-1" v-for="tagItem in articleVo.article.tags" :type="getRandomElTagType()"
            :key="tagItem.tagId">{{ tagItem.tag }}</el-tag>
        </div>
      </div>
    </div>
    ...
    <div class="article-change direction" v-if="articleVo.other && articleVo.other.flip">
      <a class="step-btn--prev" :href="articleVo.other.flip.prevHref" v-if="articleVo.other.flip.prevShow">
        <div class="article-change-item">
          <svg t="1670064682276" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg"
            p-id="9458" width="32" height="32">
            <path
              ...
              p-id="9459" fill="#ffffff"></path>
          </svg>
        </div>
      </a>
      <a class="step-btn--next" :href="articleVo.other.flip.nextHref" v-if="articleVo.other.flip.nextShow">
        <div class="article-change-item">
          <svg t="1670064662589" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg"
            p-id="8352" width="32" height="32">
            <path
    ...
    <div
      v-if="articleVo.other && articleVo.other.readType === 3 && !(global.user != null && global.user.starStatus == 'FORMAL')">
      <div class="needlock">
        <a class="btn-readmore no-login underline" href="#" target="_blank" data-target="#registerModal"
          data-toggle="modal">
    ...
    <div class="article-heart bg-color-white">
      ...
      <div class="praise-photos">
        <p class="approval-tips-line" id="praiseDesc" data-defult-text="真诚点赞 诚不我欺">
          {{ praiseCnt > 0 ? praiseCnt + '人已点赞' : '真诚点赞 诚不我欺' }}
        </p>
        <div class="approval-box" id="praiseUsers">
          <RouterLink class="g-user-popover approval-img" :to="'/user/' + item.userId"
            v-for="(item, id) in praisedUsers" :key="id">
            <img :src="item.avatar">
          </RouterLink>
        </div>
      </div>
  ...
  <el-dialog v-if="global.user && global.user.id == articleVo.article.author" v-model="deleteDialog" title="删除提醒"
    width="500" center>
    <div>
  ...
  if (praised.value) {
    ...
    doGet<CommonResponse>(ARTICLE_LIKE_COLLECT_URL, {
      ...
      .then((response) => {
        praiseCnt.value++
        praised.value = true
        showPraisePlus.value = true
        setTimeout(() => { showPraisePlus.value = false }, 1000)
  ...
  if (collected.value) {
    ...
    doGet<CommonResponse>(ARTICLE_LIKE_COLLECT_URL, {
      ...
      .then((response) => {
        ...
        showCollectPlus.value = true
        setTimeout(() => { showCollectPlus.value = false }, 1000)
      })
      .catch((error) => {
        console.error(error)
      })
...
const scrollToComment = () => {
  const commentSection = document.getElementById('comment-list-wrapper')
  if (commentSection) {
